...
${sliceText(escapedAppName, metadata.status.device_slice).replaceAll('\n', ' <br/>\n')}
        ...
        // 显示截图
        if (device.fields && device.fields.screenshot) {
            ...
            if (screenshotDisplay && screenshotImg) {
                screenshotImg.src = `/api/device/screenshot/${device.fields.screenshot}?t=${Date.now()}`;
                screenshotDisplay.style.display = 'block';
                if (screenshotTime) {
                    screenshotTime.textContent = `截图时间: ${getFormattedTime(new Date(device.last_updated * 1000))}`;
                }
            }
        }
    }
